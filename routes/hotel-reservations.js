    ...
    try {
        ...
        const reservation = await pool.query(`
            SELECT 
                ...
                h.hotel_name,
                h.hotel_name_en,
                h.reservation_email as hotel_email,
...
router.post('/:id/confirm', async (req, res) => {
    ...
    try {
        ...
        let mainConfirmationNumber = null;
        ...
        if (Array.isArray(rooms)) {
            for (const room of rooms) {
                ...
                // 첫 번째 확정번호를 메인 확정번호로 저장
                if (!mainConfirmationNumber && confirmationNumber) {
                    mainConfirmationNumber = confirmationNumber;
                }
        ...
        // ⭐ 예약 테이블에도 확정번호 저장 (첫 번째 객실의 확정번호)
        await client.query(`
            UPDATE hotel_reservations
            SET status = 'confirmed', 
                ...
                updated_at = NOW()
            WHERE id = $2
        `, [mainConfirmationNumber, id]);
        
        console.log(`✅ 예약 확정번호 저장: ${mainConfirmationNumber}`);
